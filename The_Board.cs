   ...
   private void SetUp()
   {
    ...
    private IEnumerator FillBoardCo()
    {
        ...
        while(MatchesOnBoard())
        {
            streakValue ++;
            DestroyMatches();
            yield return new WaitForSeconds(2 * refillDelay); //this was .3f , i think this controls how fast they get destroyed from getitng formed by board refill
            
        }
        findMatches.currentMatches.Clear();
        currentDot = null;
        //yield used to be here
        if(IsDeadlocked())
        {
            ShuffleBoard();
            Debug.Log("Deadlocked!");
        }
        yield return new WaitForSeconds(refillDelay);// this yield used to be couple lines above
        currentState = GameState.move;
        streakValue = 1;
    }

    //This and the 3 methods below it are helper methods for board DEADLOCK
    ...
    private bool CheckforMatches()
    {
        for(int i = 0; i < width; i++)
        {
            for(int j = 0; j < height; j++)
            {
                if(allDots[i,j] != null)
                {   
                    //Make sure that on and two to the right are in the board
                    if(i < width - 2)
                    {
    ...
    //This is to shuffle the board if it's deadlocked!
    private void ShuffleBoard()//this used to be type "void" not "IEnumerator"
    {
